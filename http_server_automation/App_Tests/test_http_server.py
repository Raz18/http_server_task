...
from App_Tests.conftest import BASE_URL


class TestHTTPServerManagement:
    def setup_class(self):

        #Setup a shared list for managing server IDs across tests.

        self.active_servers = []

    def test_start_http_server(self, server_setup):
        """
        Test starting an HTTP server and verifying it is running.
        """
        url = f"{server_setup}/{BASE_URL}"
        ...
        self.active_servers.append(result["server_id"])
    ...
    def test_stop_http_server(self, server_setup):
        """
         Test stopping an active HTTP server
        """
        url = f"{server_setup}/{BASE_URL}"
        #print(url)
        data = {
    ...
    def test_max_servers(self, server_setup):
        """
        Test starting the maximum number of HTTP servers (10) and ensure the 11th attempt fails, then cleanup phase
        """
        url = f"{server_setup}/{BASE_URL}"
